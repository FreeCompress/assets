let gifsicle = {
    tool: {
        ...
        testType(A) {
            return A instanceof Element ? "element" : Object.prototype.toString.call(A).replace(/\[object\s(.+)\]/, "$1").toLowerCase()
        },
        async textToUrl(A) {
            return this.worker()
        },
        ...
        loadOne(B) {
            return new Promise(async(I,A)=>{
                ...
                if (["string"].includes(g))
                    fetch(B).then(A=>{
                        if (200 !== A.status)
                            throw "<" + B + "> Url error!!!" + this.errorLink();
                        return A.arrayBuffer()
                    }
                    ).then(A=>I(A));
                else if (["blob", "file"].includes(g))
                    new Response(B).arrayBuffer().then(A=>I(A));
                else {
                    if (!["arraybuffer"].includes(g))
                        throw "<input.file> types:" + g + ", only supports Url, blob, file, arraybuffer" + this.errorLink();
                    I(B)
                }
            }
            )
        ...
        loadFile(B) {
            return new Promise(async(I,A)=>{
                var g = this.testType(B);
                if ("array" !== g)
                    throw "<input> types:" + g + ", only supports Array" + this.errorLink();
                if (0 === B.length)
                    throw "<input> the content can not be blank" + this.errorLink();
                g = B.map(A=>this.loadOne(A.file));
                Promise.all(g).then(function(g) {
                    var A = B.map((A,I)=>(A.file = g[I],
                    A));
                    I(A)
                }).catch(function(A) {})
            }
        ...
        loadFolder(B) {
            return new Promise(async(A,I)=>{
                var g = this.testType(B);
                if (!["array"].includes(g))
                    throw "<folder> types:" + g + ", only supports Array" + this.errorLink();
                A(B)
            }
            )
        }
    ...
    run(t={}) {
        return new Promise(async(Q,A)=>{
            let {input: I=[], command: g="", folder: B=[], isStrict: C=!1, start: E=A=>{}
            } = t;
            var i = await this.tool.textToUrl();
            let o = new Worker(i);
            var D = this.tool.loadCommand(g)
              , w = await this.tool.loadFile(I)
              , e = (E(w),
            await this.tool.loadFolder(B));
        //    console.log(D),
        //    console.log(w),
        //    console.log(i),
            o.postMessage({
